    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        {(bestDaySessions > 0 || bestMonthSessions > 0 || bestWeekSessions > 0 || bestYearSessions > 0) && (
          <div className="pt-4 border-t border-muted-foreground/20">
            <p className="text-xl font-semibold mb-4 text-center">Allzeit-Rekorde:</p> {/* Added colon */}
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-center">
              <div>
                <p className="text-lg font-medium">Tag:</p> {/* Removed "Bester" */}
                <p className="text-2xl font-bold text-accent">{bestDaySessions}</p>
              </div>
              <div>
                <p className="text-lg font-medium">Woche:</p> {/* Removed "Beste" */}
                <p className="text-2xl font-bold text-accent">{bestWeekSessions}</p>
              </div>
              <div>
                <p className="text-lg font-medium">Monat:</p> {/* Removed "Bester" */}
                <p className="text-2xl font-bold text-accent">{bestMonthSessions}</p>
              </div>
              <div>
                <p className="text-lg font-medium">Jahr:</p> {/* Removed "Bestes" */}
